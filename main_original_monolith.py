...
def resume_to_text(file: UploadFile) -> str:
    ...
    # Check both content_type and filename for file type detection
    is_pdf = file.content_type == "application/pdf" or (file.filename and file.filename.lower().endswith('.pdf'))
    is_docx = file.content_type == "application/vnd.openxmlformats-officedocument.wordprocessingml.document" or (file.filename and file.filename.lower().endswith(('.docx', '.doc')))
...
async def check_resume(
    ...
    """
    Main endpoint for resume analysis (multipart form version - DEPRECATED due to API Gateway corruption)
    """
    raise HTTPException(status_code=400, detail="Please use the base64 upload endpoint due to API Gateway file corruption issues")

@app.post("/api/check-resume-base64") 
async def check_resume_base64(request: dict):
    """
    Base64 endpoint for resume analysis - bypasses API Gateway multipart corruption
    Expected format: {
    ...
    print(f"📁 Base64 upload received")
    ...
    if not file_content_b64:
        raise HTTPException(status_code=400, detail="Missing file_content")
    ...
    try:
        ...
        print(f"📁 Decoded file: {filename}, size: {len(file_content)} bytes")
    ...
    <body>
        ...
        <script>
            ...
            async function analyzeResume() {
                ...
                try {
                    // Convert file to base64 to bypass API Gateway multipart corruption
                    ...
                    const response = await fetch('/api/check-resume-base64', {
                        method: 'POST',
                        headers: {
