...
def check_resume(  # CRITICAL: Use synchronous function not async
    file: UploadFile = File(...),
    payment_token: Optional[str] = Form(None)
):
    """Main endpoint for resume analysis - Lambda-optimized version"""
    
    print(f"📁 File upload received: {file.filename}, type: {file.content_type}")
...
def check_resume_options():  # CRITICAL: Use synchronous function not async
    """Handle CORS preflight"""
    return {"message": "OK"}

@app.get("/api/health")
def health_check():  # CRITICAL: Use synchronous function not async
    """Health check endpoint"""
    return {"status": "healthy", "service": "resume-health-checker-working"}

@app.options("/api/health")
def health_options():  # CRITICAL: Use synchronous function not async
    """Handle CORS preflight"""
    return {"message": "OK"}
